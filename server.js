    ...
    socket.on("message", (message) => {
        try {
            ...
            if (data.type === "createLobby") {
                ...
                joinLobby(socket, lobby, data.username);
            ...
            if (socket.lobbyCode !== null) {
                ...
                if (data.type === "changeHead") {
                    ...
                    const currentHead = Number(player.head ?? 0);
                    const nextHead = (currentHead + 1) % 8;
                ...
                if(data.type === "prompts") {
                    ...
                    if (Object.keys(lobby.prompts).length === lobby.players.length) {
                        ...
                        const replace = shuffle(REPLACE_WORDS).slice(0, lobby.prompts.length * PAINTINGS_PER_PERSON); // replace 2 when implementing settings
                ...
                if(data.type === "paintings") {
                    ...
                    if (lobby.paintings.length / PAINTINGS_PER_PERSON == lobby.players.length) {
                        ...
                        function getHints() {
                            const l = Math.max(Math.ceil(lobby.paintings.length / 4), 2);
                            const h = shuffle(lobby.paintings);
...
function joinLobby(socket, lobby, name) {
    ...
    const player = {
        ...
        name: name == "" ? "Player" + playerId : name,
        head: 0,
